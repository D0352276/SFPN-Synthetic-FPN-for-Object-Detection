...
from modules import ConvBN,CSLFPN,LayerExpansion
import math
...
class CSLHead(tf.Module):
    def __init__(self,
        ...
        self._syn_output_layers=syn_output_layers
        self._name=name
    @tf.Module.with_name_scope
    ...
    def __call__(self,input_ts_list):
        output_ts_list=[]
        if(self._syn_output_layers==False and len(input_ts_list)==9):
            input_ts_list=[input_ts_list[0],input_ts_list[4],input_ts_list[8]]
        elif(self._syn_output_layers==False and len(input_ts_list)==5):
            input_ts_list=[input_ts_list[0],input_ts_list[2],input_ts_list[4]]
        ...
        if(self._nms_type=="category_nms"):
            ...
            output_ts=self._PostProcessPredTs(output_ts)
        elif(self._nms_type=="none"):
            output_ts=output_ts
            output_ts=self._PostProcessPredTs(output_ts)
        return output_ts

class CSLYOLOBody(tf.Module):
    def __init__(self,fpn_filters=96,repeat=3,out_layers=3,name="cslyolobody"):
    ...
    @tf.Module.with_name_scope
    def _Build(self):
        self._l1_cspg=ConvBN(filters=self._fpn_filters,kernel_size=(3,3),name=self._name+"_l1_cspg")
        self._l3_cspg=ConvBN(filters=self._fpn_filters,kernel_size=(3,3),name=self._name+"_l3_cspg")
        self._l5_cspg=ConvBN(filters=self._fpn_filters,kernel_size=(3,3),name=self._name+"_l5_cspg")
        self._layer_exapnd=LayerExpansion(out_layers=self._out_layers,name=self._name+"_layer_exapnd")
    ...
    def __call__(self,bacbone_l1,bacbone_l2,bacbone_l3):
        ...
        l2=self._l3_cspg(orig_l2)
        l3=self._l5_cspg(orig_l3)
        out_ts_list=self._layer_exapnd([l1,l2,l3])
        out_ts_list=self._cslfpn(out_ts_list)
        return out_ts_list
...
class CSLLoss(tf.Module):
    ...
    def __call__(self):
        def _CSLLoss(true_y,pred_y):
            ...
            cnfd_loss_2=self._ConfidenceFocalLoss(pred_y,true_mask,ignore_mask,true_wht)
            cnfd_loss=cnfd_loss_1*0.5+cnfd_loss_2*0.5
