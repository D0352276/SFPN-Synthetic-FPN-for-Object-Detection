...
class MultiDataGenerator:
    def __init__(self,imgs_dir,jsons_dir,
                 img_hw,out_hw_list,anchors_list,
                 labels,batch_size=-1,max_queue=10,data_type="jpg",print_bool=True):
        self._data_gen=DataGenerator(imgs_dir,jsons_dir,img_hw,
                                     out_hw_list,anchors_list,labels,
                                     batch_size,data_type,print_bool)
        self._process_num=int(cpu_count()/2)
        self._pool=Pool(self._process_num)
        self._batch_data_queue=Manager().Queue()
    ...
    def Start(self):
        self._pool.map_async(self._ReadFunction,[None for i in range(self._process_num)])
        return 
    def Stop(self):
